...
|----------|---------|-----------|
| `TGBP.sol` | Base stable token used for trading and rewards | DEFAULT_ADMIN_ROLE, MINTER_ROLE |
...
| `OrderBookDEX.sol` | On-chain limit order book engine | Trades TGBP ↔ EquityTokens, partial fills |
| `Dividends.sol` | Snapshot-based payouts in TGBP to stock holders | DIVIDEND_ROLE |
| `FeePool.sol` | Tracks trading volumes; pays 3 TGBP per epoch to top trader | REWARD_MANAGER_ROLE |
...
3. User connects wallet: Link address → receives 1 000 000 TGBP airdrop if first time.  
...
6. After epoch ends, `finalizeEpoch()` identifies top trader → transfers 3 TGBP reward.  
7. Admin declares dividend: Dividends.declare(symbol, totalTGBP, claimUntil); users claim pro-rata.  
...
- Asset conservation: TGBP + EquityToken balances before/after trades must balance ± fee.  
